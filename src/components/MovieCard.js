...
const MovieCard = ({ poster_path, movieId }) => {
  ...
  return (
    <div
      ...
      className="relative w-48 cursor-pointer shadow-3xl hover:border border-white hover:scale-125 transition-transform duration-300 rounded-lg"
    >
      <div onClick={handleInfoClick} className="absolute p-1 right-1 top-1 text-white text-xl opacity-85 "><MdOutlineInfo /></div>
      <img className={"hover:rounded-lg"} alt="movie loading.." src={IMG_CDN_LINK + poster_path} />
    </div>
  );
};
